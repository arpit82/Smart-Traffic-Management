...
def mainfunc(ref_point,file_name):
    print(ref_point)
    ...
    output_format = 'XVID'
    video = file_name
    output_filename = "Traffic_Violation_%s.avi" % (datetime.datetime.now().strftime('%d%m%Y%H%M'))
    output = './data/output/Traffic Violations/'+output_filename
    size = 416
    ...
    #best_img=[[0,0,0] for i in range(200)]
    downCount = 0
    upCount = 0 
    id_list=[]
    start=int(time.time())
    ...
    while True:
        ...
        if(red_light==0):
            cv2.line(img, (ref_point[0][0], ref_point[0][1]), (ref_point[1][0], ref_point[1][1]), (0, 255, 0), 2)
        elif(red_light==1):
            cv2.line(img, (ref_point[0][0], ref_point[0][1]), (ref_point[1][0], ref_point[1][1]), (0, 0, 255), 2)
        ...
        for track in tracker.tracks:
            ...
            #obj_dict[obj_id]=centroids[obj_id]
            ...
            vimages_path="G:\\object tracking\\smart traffic management\\data\output\\Traffic Violations\\Vehicle Images\\"
            if(direction>0 and new > ref_point[0][1] and olds[0] < ref_point[0][1] and centroidsx[obj_id] >= ref_point[0][0] and centroidsx[obj_id] < ref_point[1][0] and red_light==1):
                if(idCounter[obj_id]!=1):
                    downCount += 1
                    crop = img[y1-5:y2+5,x1-5:x2+5]
                    try:
                        imgname=vimages_path+"image"+str(obj_id)+'.jpg'
            ...
            elif(direction<0 and new < ref_point[0][1] and olds[0] > ref_point[0][1] and centroidsx[obj_id] >= ref_point[0][0] and centroidsx[obj_id] <= ref_point[1][0] and red_light==1 ):
                if(idCounter[obj_id]!=1):
                    upCount += 1
                    crop = img[y1-5:y2+5,x1-5:x2+5] 
                    try:
